...
def get_columns_config(contract_start_date, contract_end_date, number_type: Literal['Float', 'Integer'], df_title="", header=""):
    ...
    columns_config = {date: st.column_config.NumberColumn(
        date, format="%.0f" if number_type == 'Integer' else "%.2f", width="small") for date in contract_range}
    columns_config[df_title] = st.column_config.TextColumn(
        header, width="medium", disabled=True)
...
def fetch_data(project, person, start_date, end_date):
    ...
    sheet = st.session_state.sheets.query(
        'person == @person and date >= @start_date and date <= @end_date')
    real_work = st.session_state.real_work.query(
        'person == @person and date >= @start_date and date <= @end_date')
    planned_work = st.session_state.planned_work.query(
        'person == @person and date >= @start_date and date <= @end_date and project == @project["name"]')

    sheet = sheet.merge(real_work.query('project == @project["name"]')[
                        ['date', 'hours']], on="date", how="left").rename(columns={'hours': 'Horas Reais'})
...
def sheet_widget(project):
    ...
    if person := st.selectbox("Selecionar Membro", options=contracts['person'].unique()):
        ...
        sheet, activities, real_work, planned_work = fetch_data(
            project, person, start_date, end_date)

        st.subheader("Folha de Horas")  # Folha de Horas

        sheet = sheet.loc[:, ~sheet.columns.duplicated(keep='last')]
        modifications = st.data_editor(
            sheet.loc[['Jornada Diária', 'Dias Úteis', 'Faltas', 'Férias', "Horas Reais"]].apply(
        ...
        modifications.loc['Horas Trabalhadas'] = modifications.loc['Jornada Diária'].fillna(
            0) * modifications.loc['Dias Úteis'].fillna(0) - modifications.loc['Faltas'].fillna(0) - modifications.loc['Férias'].fillna(0)
        modifications.loc['FTE'] = (modifications.loc['Horas Reais'].fillna(0) / (modifications.loc['Jornada Diária'].fillna(
            0) * modifications.loc['Dias Úteis'].fillna(0) - modifications.loc['Férias'].fillna(0)).replace(0, np.nan)).fillna(0)
        ...
        if st.toggle("Tabela Vertical"):
            ...
            for row in salary_df.itertuples(index=False):
                ...
                modifications.loc[["Salário", "SS"], date_range(date, end_date)] = {
                    'Salário': row.Salário, 'SS': row.SS}
            ...
            modifications.loc[['Salário', 'SS']] = st.data_editor(
                ...
                column_config=get_columns_config(
        ...
        modifications.loc['Custo Aproximado'] = round_down(
            (
                round_down(
                    ...
                    (modifications.loc['Jornada Diária'] *
                     modifications.loc['Dias Úteis']).replace(0, np.nan),
                    4
                ) *
        ...
        if save:  # Update Folha de Horas
            ...
            to_update['date'] = pd.to_datetime(
                to_update['date'], format='%b/%y')
            to_update['person'] = person

            st.session_state.sheets = st.session_state.sheets.query(
                '(person != @person) or (date < @start_date or date > @end_date)')
            st.session_state.sheets = pd.concat([st.session_state.sheets, to_update[[
                                                'person', 'date', 'Jornada Diária', 'Faltas', 'Férias', 'Salário', 'SS', 'Custo Aproximado']]])
            ...
            st.session_state.real_work = st.session_state.real_work.query(
                '(person != @person) or (project != @project["name"]) or (date < @start_date or date > @end_date)')
            st.session_state.real_work = pd.concat(
                [st.session_state.real_work, to_update[['person', 'project', 'date', 'hours']]])
            st.session_state.working_days = st.session_state.working_days.query(
                'project != @project["name"] or (date < @start_date or date > @end_date)')
            st.session_state.working_days = pd.concat(
                [st.session_state.working_days, to_update[['project', 'date', 'day']]])

        st.dataframe(
            modifications.loc[['Custo Aproximado']].style.format(
                "{:.2f} €").map(format_zeros),
            use_container_width=True,
            column_order=columns_order,
            column_config=get_columns_config(start_date, end_date, 'Integer')
        ...
        planned_work = planned_work.merge(
            activities[['activity', "wp", 'trl']], on="activity", how="left")
        ...
        for wp in planned_work['wp'].unique():
            ...
            wp_sheet_modifications = st.data_editor(
                ...
                column_config=get_columns_config(
                    start_date, end_date, 'Integer', "activity", wp),
            )

            wp_sheet = pd.concat([wp_sheet, wp_sheet_modifications])

        summary_wp = wp_sheet.reset_index(names="activity").merge(
            activities[["wp", "activity"]], how="left", on="activity")
        ...
        wp_sum_container.dataframe(
            ...
            column_order=columns_order,
            column_config=get_columns_config(
                start_date, end_date, 'Integer', 'wp', 'Tabela Resumo'),
        )

        st.subheader("Outros Projetos")  # Outras Atividades Section

        other_projects = real_work.query(
            'date >= @start_date and date <= @end_date and project != @project["name"]')[['date', 'hours', 'project']]
        other_projects['date'] = other_projects['date'].apply(
            lambda x: pd.to_datetime(x).strftime('%b/%y'))

        non_removable_projects = real_work.query(
            'project in @st.session_state.projects["name"]')['project'].unique()
        removable_projects = real_work.query(
            'project not in @st.session_state.projects["name"]')['project'].unique()
        projects_suggestion = st.session_state.real_work.query(
            'project not in @st.session_state.projects["name"]')['project'].unique()

        project_list = st_tags(
            list(other_projects.query(
                'project not in @st.session_state.projects["name"]')['project'].unique()),
        ...
        other_projects = other_projects.pivot_table(
            index='project', columns='date', values='hours')
        other_projects = pd.concat(
            [pd.DataFrame(columns=sheet.columns), other_projects])

        for project_ in project_list:
            if project_ not in other_projects.index:
                other_projects.loc[project_] = None

        other_projects = other_projects.query(
            'index in @project_list or index in @non_removable_projects')
        ...
        other_projects = st.data_editor(
            ...
            column_config=get_columns_config(
                start_date, end_date, 'Integer', header="Projeto"),
        )

        remainder = pd.DataFrame(columns=sheet.columns)
        remainder.loc['Outras Atividades'] = modifications.loc['Horas Trabalhadas'] - \
            other_projects.sum(axis=0) - \
            modifications.loc['Horas Reais'].fillna(0)
        ...
        modifications.loc["Horas Restantes"] = modifications.loc['Horas Trabalhadas'] - \
            other_projects.sum(axis=0) - \
            modifications.loc['Horas Reais'].fillna(0)

        modifications_container.dataframe(
            modifications.loc[['Horas Trabalhadas', 'FTE', 'Horas Planeadas', "Horas Restantes"]].style.format(
                "{:.2f}").map(format_zeros_and_negatives),
            use_container_width=True,
            column_order=columns_order,
            column_config=get_columns_config(start_date, end_date, 'Integer')
        )
        ...
        if save:  # Real Work (other projects) update
            ...
            st.session_state.real_work.query(
                '~ (person == @person and project != @project["name"] and date >= @start_date and date <= @end_date)', inplace=True)
            st.session_state.real_work = pd.concat(
                [st.session_state.real_work, df])

        if not wp_sheet.empty:

            if save:  # Planned work update
                wp_sheet = wp_sheet.reset_index(names='activity')
                wp_sheet = wp_sheet.melt(
                    id_vars='activity', var_name='date', value_name='hours')
                wp_sheet['date'] = pd.to_datetime(
                    wp_sheet['date'], format='%b/%y')
                ...
                for act in st.session_state.activities.query('project == @project["name"]').itertuples(index=False):
                    wp_sheet = wp_sheet.query(
                        'not (activity == @act.activity and (date < @get_first_date(@act.real_start_date) or date > @get_first_date(@act.real_end_date)))')
                ...
                st.session_state.planned_work = st.session_state.planned_work.query(
                    '(person != @person) or (project != @project["name"]) or (date < @get_first_date(@start_date) or date > @get_first_date(@end_date))')
                st.session_state.planned_work = pd.concat(
                    [st.session_state.planned_work, wp_sheet])
                st.session_state.notification = {
                    "message": "Atualizado", "type": "success"}
                reset_key()

            horas_trabalhaveis = (
                modifications.loc['Jornada Diária'] * modifications.loc['Dias Úteis']).fillna(0)
            sum_wp = wp_sheet.sum()
            ...
            wp_sheet = (
                (wp_sheet / sum_wp * modifications.loc['Horas Reais']) / horas_trabalhaveis).fillna(0)
            ...
            wp_sheet = wp_sheet.reset_index(names="activity")
            wp_sheet = wp_sheet.merge(
                activities[['activity', 'wp', 'trl']], on="activity", how="left")

            st.subheader("Afetação WP e TRL")

            df = wp_sheet.drop(columns=['activity']).groupby(
                ['wp', 'trl']).sum()
            df = df * 100
            df = df.style.format("{:.2f} %")
            df = df.map(format_zeros)
