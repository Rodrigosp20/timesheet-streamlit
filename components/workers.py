...
def generate_sheets(start_date, end_date):
    ...
    sheets = st.session_state.sheets.query('date >= @start_date and date <= @end_date').sort_values(by="date")
    days = st.session_state.working_days.query('date >= @start_date and date <= @end_date').drop_duplicates(subset="date")
    ...
    for person in sheets['person'].unique():
        ...
        work = st.session_state.real_work.query('date >= @start_date and date <= @end_date and person == @person')
        ...
        for col_num, row in enumerate(sheet.itertuples(index=False), start=4):
            ...
            for line, project in enumerate(projects, start=13):
                try:
                    ws[f'{col}{line}'] = date_work.loc[date_work['project'] == project, "hours"].iloc[0]
                except:
                    pass
...
def update_workers(df: pd.DataFrame):
    ...
    for person in st.session_state.persons['name']:
        ...
        if person not in list(df['name']):
            ...
            st.session_state.sheets = st.session_state.sheets.query('person != @person')
            st.session_state.real_work = st.session_state.real_work.query('person != @person')
            st.session_state.planned_work = st.session_state.planned_work.query('person != @person')
            st.session_state.contracts = st.session_state.contracts.query('person != @person')
    #update workers
...
def workers_widget(): 
    ...
    with st.expander("Gerar Folhas de Afetação p/ funcionario"):
