...
def generate_timesheet(project, start_date, end_date):
    wb = load_workbook("assets/Timesheet_Template.xlsx")
    ...
    for i, wp in enumerate(acts_df['wp'].unique()):
        ...
        for j, act in enumerate(acts_df.query('wp == @wp').itertuples(index=False)):
            ...
            for col in range(7, 55):
                date = get_first_date(pd.Timestamp(
                    project['start_date'] + relativedelta(months=col-7)))
    ...
    for line, contract in enumerate(contracts.itertuples(index=False), start=9):
        ...
        # Get Profile data
        previous_ss = None
        sh = st.session_state.sheets.query(
            'person == @contract.person and date >= @get_first_date(@contract.start_date) and date <= @get_first_date(@contract.end_date)')
        planned = st.session_state.planned_work.query(
            'person == @contract.person and project == @project["name"] and date >= @get_first_date(@contract.start_date) and date <= @get_first_date(@contract.end_date)')
        real = st.session_state.real_work.query(
            'person == @contract.person and date >= @get_first_date(@contract.start_date) and date <= @get_first_date(@contract.end_date)').sort_values(by=['project', 'date'])
                ...
                for i, act_list in enumerate(acts.values()):
                    ...
                    for line, act in enumerate(act_list, line):
                        if not (act_row := p_sh.query('activity == @act')).empty:
                            sheet.cell(
                                line, col).value = act_row.iloc[0]['hours']
...
def update_project(project, file, other_activities=True):
    ...
    for contract in contracts.itertuples(index=False):
        ...
        date_range = (df.columns >= pd.to_datetime(contract.start_date)) & (
            df.columns <= pd.to_datetime(contract.end_date))
        contract_range = df.columns[date_range]

        try:
            sheet = df.loc[['Jornada diária', 'N.º de dias \núteis',
                            'Faltas (horas/mês)', 'Férias (horas/mês)', 'Salário atualizado (€)', 'SS'], contract_range].fillna(0)
        except:
            raise Exception(
                f"Folha {sheet}: Erro na leitura da folha de horas")
        ...
        try:
            sheet[['Jornada Diária', 'day', 'Faltas', 'Férias', 'Salário']] = sheet[[
                'Jornada Diária', 'day', 'Faltas', 'Férias', 'Salário']].astype('float64')
        except:
            raise Exception(
                f"Erro a ler a folha {sheet} - Não foi possível converter a folha com Números Inválidos")
        ...
        try:
            planned_work = df.loc[activities['activity'].unique(),
                                  contract_range]
        except:
            raise Exception(
                f"Folha {sheet}: Não foram encontradas as atividades")

        planned_work = planned_work[~planned_work.index.duplicated()].fillna(0)

        planned_work = planned_work.reset_index(names="activity").melt(
            id_vars='activity', var_name='date', value_name='hours')
        ...
        try:
            real_work = df.loc[df.index.str.contains(
                'Horas Reais').fillna(False), contract_range]
        except:
            raise Exception(
                f"Folha {sheet}: Não foram encontradas as Horas Reais do Projeto")

        try:
            index_position = df.index.get_loc('Outras atividades')
            other_activities = df.iloc[index_position -
                                       3:index_position].loc[:, contract_range]
        ...
        real_work = real_work.reset_index(names="project").melt(
            id_vars='project', var_name='date', value_name='hours')

        real_work['person'] = contract.person
        real_work['date'] = pd.to_datetime(real_work['date'])
        real_work.loc[real_work['project'].str.contains(
            'Horas Reais'), 'project'] = project["name"]

        real_works = pd.concat([real_works, real_work])

    st.session_state.sheets = pd.concat([st.session_state.sheets, sheets])
    st.session_state.sheets = st.session_state.sheets.drop_duplicates(
        subset=["person", "date"], keep='last')

    st.session_state.real_work = pd.concat(
        [st.session_state.real_work, real_works])
    st.session_state.real_work = st.session_state.real_work.drop_duplicates(
        subset=["person", "project", "date"], keep='last')

    st.session_state.working_days = pd.concat(
        [st.session_state.working_days, working_days])
    st.session_state.working_days = st.session_state.working_days.drop_duplicates(
        subset=["project", "date"], keep='last')

    for act in activities.itertuples(index=False):
        planned_works = planned_works.query(
            '~ (activity == @act.activity and (date < @act.real_start_date or date > @act.real_end_date))')

    st.session_state.planned_work = pd.concat(
        [st.session_state.planned_work, planned_works])
    st.session_state.planned_work = st.session_state.planned_work.drop_duplicates(
        subset=["project", "person", "activity", "date"], keep='last')
...
def delete_project(project):
    st.session_state.projects = st.session_state.projects.query(
        'name != @project["name"]')
    st.session_state.activities = st.session_state.activities.query(
        'project != @project["name"]')
    st.session_state.contracts = st.session_state.contracts.query(
        'project != @project["name"]')
    st.session_state.working_days = st.session_state.contracts.query(
        'project != @project["name"]')
...
def update_project_dates(project, start, end):
    st.session_state.projects.loc[st.session_state.projects['name'] == project["name"], [
        "start_date", "end_date"]] = [start, end]

    project_range = pd.date_range(
        start=get_first_date(start), end=end, freq='MS')
    business_days = []
    for month_start in project_range:
        business_days.append(len(pd.date_range(
            start=month_start, end=month_start + pd.offsets.MonthEnd(), freq=pd.offsets.BDay())))

    st.session_state.working_days = pd.concat([st.session_state.working_days, pd.DataFrame(
        {"project": project["name"], "day": business_days, "date": project_range})])
    st.session_state.working_days = st.session_state.working_days.drop_duplicates(
        subset=["project", "date"])
...
def get_salary_info(person, start, end):
    ...
    sheet = st.session_state.sheets.query(
        'person == @person and date >= @start and date <= @end').sort_values(by="date")
...
def generate_sheets(project, start, end):
    ...
    wb = load_workbook('assets/Timesheet_ROC_Template.xlsx')
    template = wb['sheet']

    start = get_first_date(start)
    end = get_last_date(end)

    initial_diff = (start.year - project["start_date"].year) * \
        12 + start.month - project["start_date"].month
    ...
    for contract in contracts.itertuples(index=False):
        ...
        salary = get_salary_info(
            contract.person, project["start_date"], project["end_date"])
        ...
        df = st.session_state.sheets.query(
            'person == @contract.person and date >= @start and date <= @end')
        df = df.merge(working_days[['date', 'day']], on="date", how="left").rename(
            columns={'day': 'Dias Úteis'})
        real_work = st.session_state.real_work.query(
            'person == @contract.person and date >= @start and date <= @end')
        ...
        planned_work = st.session_state.planned_work.query(
            'person == @contract.person and project == @project["name"] and date >= @start and date <= @end')

        df_t = df.drop(columns='person').set_index('date')
        df_t = df_t.transpose()

        planned_work = planned_work.pivot(
            index="activity", columns="date", values="hours")

        horas_trabalhaveis = (
            df_t.loc['Jornada Diária'] * df_t.loc['Dias Úteis']).fillna(0)
        ...
        planned_work = planned_work.groupby(
            ["wp", "trl"]).sum().drop(columns="activity")

        planned_work.columns = planned_work.columns.map(
            lambda col: col.strftime("%b/%y"))
        row = 23
        ...
        for date in pd.date_range(start=start, end=end, freq="MS"):
            if not (row := df.loc[df['date'] == date]).empty:
                sheet.cell(row=5, column=x,
                           value=row['Jornada Diária'].iloc[0])
                sheet.cell(row=6, column=x, value=row['Dias Úteis'].iloc[0])
                sheet.cell(row=8, column=x, value=val if (
                    val := row['Faltas'].iloc[0]) != 0 else "")
                sheet.cell(row=9, column=x, value=val if (
                    val := row['Férias'].iloc[0]) != 0 else "")
            ...
            while (wp := sheet.cell(row=row, column=3).value):
                try:
                    sheet.cell(row=row, column=x, value=planned_work.loc[(
                        wp, 'TRL 3-4'), date.strftime("%b/%y")])
                except:
                    pass
                try:
                    sheet.cell(
                        row=row+1, column=x, value=planned_work.loc[(wp, 'TRL 5-9'), date.strftime("%b/%y")])
                except:
                    pass
        ...
        blueFill = PatternFill(start_color='D9E1F2',
                               end_color='D9E1F2', fill_type='solid')
        blueFont = Font(color="0070C0", name="Aptos Narrow", bold=True)
        sheet.conditional_formatting.add('E23:BL42', CellIsRule(
            'greaterThan', formula=['0'], fill=blueFill, font=blueFont))
...
def generate_input(project, start_date, end_date):
    wb = load_workbook('assets/Timesheet_Client_Template.xlsx')
    template = wb['sheet']
    ...
    for person_line, contract in enumerate(project_contracts.itertuples(index=False), 9):
        ...
        for i, col in enumerate(range(column_index_from_string('E'), column_index_from_string('BA'))):
            ...
            sheet.cell(row=1, column=1,
                       value=f"{contract.person} - {contract.profile}")

            if not (row := df.query('date == @date')).empty:

                sheet.cell(row=5, column=col,
                           value=row['Jornada Diária'].iloc[0])
                sheet.cell(row=6, column=col, value=row['day'].iloc[0])
                sheet.cell(row=8, column=col, value=val if (
                    val := row['Faltas'].iloc[0]) != 0 else "")
                sheet.cell(row=9, column=col, value=val if (
                    val := row['Férias'].iloc[0]) != 0 else "")
            ...
            for offset, wp_acts in enumerate(acts):
                ...
                for i, act in enumerate(wp_acts):
                    ...
                    if not (row := df_planned.query('activity == @act and date == @date')).empty:
                        ...
                        sheet.cell(row=line, column=col, value=val if (
                            val := row['planned_hours'].iloc[0]) != 0 else "")
                        sheet.cell(row=line + 112, column=col, value=val if (val :=
                                   row['real_hours'].iloc[0]) != 0 else "")
...
def generate_pay_sheets(project, file, order_by, start, end, df_team, df_trl):
    ...
    sheets = st.session_state.sheets.query(
        'person in @df_team["equipa"].unique() and date >= @start and date <= @end')
    working_days = st.session_state.working_days.query(
        'project == @project["name"]')
    sheets = sheets.merge(working_days[['date', 'day']], on="date", how="left").rename(
        columns={'day': 'Dias Úteis'})

    project_work = st.session_state.real_work.query(
        'project == @project["name"] and date >= @start and date <= @end')
    project_work = project_work.rename(columns={'hours': 'real_work'})
    planned_work = st.session_state.planned_work.query(
        'project == @project["name"] and date >= @start and date <= @end')

    sheets['horas_trabalhaveis'] = sheets['Jornada Diária'] * \
        sheets['Dias Úteis']

    planned_work = planned_work.merge(
        st.session_state.activities[['wp', 'trl', 'activity']], on="activity", how="left")

    sum_wp = planned_work.groupby(["person", "date"])[
        'hours'].sum().reset_index()
    sum_wp = sum_wp.rename(columns={'hours': 'wp_sum'})

    planned_work = planned_work.merge(
        project_work[["person", "date", "real_work"]], on=["person", "date"], how="left")
    planned_work = planned_work.merge(sum_wp[["person", "date", "wp_sum"]], on=[
                                      "person", "date"], how="left")
    planned_work = planned_work.merge(
        sheets[["date", "person", "horas_trabalhaveis"]], on=["person", "date"], how="left")

    planned_work['res'] = ((planned_work['hours'] / planned_work['wp_sum'].replace(0, np.nan) * planned_work['real_work']
                            ).replace(0, np.nan) / planned_work['horas_trabalhaveis'].replace(0, np.nan)).fillna(0)

    df_team = df_team.merge(planned_work[[
                            "person", "wp", "trl", "date", "res"]], left_on="equipa", right_on="person")
    df_team = df_team.groupby(["tecnico", "wp", "trl", "date"])[
        "res"].sum().reset_index()
...
def project_widget(project):
    ...
    with st.container(border=True):
        ...
        start_date = st.date_input(
            "Data de Inicio", key=f"project_date_initial_{st.session_state.key}", value=project['start_date'], format="DD/MM/YYYY", max_value=project['start_date'])
        end_date = st.date_input(
            "Data de Termino", key=f"project_date_final_{st.session_state.key}", value=project['end_date'], format="DD/MM/YYYY", min_value=project['end_date'])
    ...
    with st.expander("Gerar Folhas de Pagamentos"):
        ...
        if template := st.file_uploader("Template", type=".xlsx", accept_multiple_files=False):
            df_team = pd.read_excel(
                template, sheet_name="Referências", usecols="H", header=3, names=["tecnico"]).dropna()
            project_persons = st.session_state.contracts.query(
                'project == @project["name"]')["person"]
            ...
            df_team = st.data_editor(
                ...
                column_config={
                    "equipa": st.column_config.SelectboxColumn(
                        "equipa",
                        options=st.session_state.contracts.query(
                            'project == @project["name"]')["person"].unique()
                    )
                },
                disabled=["tecnico"],
            ...
            df_trl = pd.read_excel(
                template, sheet_name="Referências", usecols="E", header=3, names=["code"]).dropna()
            aux = st.session_state.inv_order_num.query(
                'project == @project["name"]')
            df_trl = pd.merge(left=df_trl, right=aux[[
                              'code', 'wp', 'trl']], how='left', left_on='code', right_on='code')
            # df_trl['wp'] = None
            # df_trl['trl'] = None
            ...
            df_trl = st.data_editor(
                ...
                column_config={
                    ...
                    "wp": st.column_config.SelectboxColumn(
                        "wp",
                        options=st.session_state.activities.query(
                            'project == @project["name"]')['wp'].unique()
                    ),
                    "trl": st.column_config.SelectboxColumn(
                        "trl",
                        options=st.session_state.activities.query(
                            'project == @project["name"]')['trl'].unique()
                    )
                },
                disabled=['code'],
                hide_index=True,
        ...
        if st.button("Gerar Excel", key="pay_excel", use_container_width=True, disabled=False if template else True):
            ...
            st.session_state.inv_order_num.drop(st.session_state.inv_order_num.query(
                'project == @project["name"]').index, inplace=True)
            st.session_state.inv_order_num = pd.concat(
                [st.session_state.inv_order_num, df_trl]).reset_index(drop=True)

            st.session_state.unsaved = True

            wb = generate_pay_sheets(
                project, template, order_by, start_date, end_date, df_team, df_trl)
            virtual_workbook = BytesIO()
            wb.save(virtual_workbook)
            virtual_workbook.seek(0)

            save_excel(virtual_workbook.getvalue(),
                       f"fpp_{project['name']}.xlsx")

    with st.expander("Exportar Timesheet"):

        client_version = st.toggle("Versão Cliente")

        filter_start, filter_end = st.slider("", min_value=get_first_date(project["start_date"]), max_value=get_first_date(
            project["end_date"]), format="MMMM/YYYY", value=(get_first_date(project["start_date"]), get_first_date(project["end_date"])), step=timedelta(weeks=4))
        ...
        if st.button("Gerar Excel", key="timesheet", use_container_width=True):
            ...
            save_excel(virtual_workbook.getvalue(
            ), f"timesheet_{project['name']}{'_cliente' if client_version else ''}.xlsx")
