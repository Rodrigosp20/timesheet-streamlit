...
def create_new_project(name, start, end):
    ...
    st.session_state.projects.loc[len(st.session_state.projects)] = [
        name, start, end, None]
    ...
    st.session_state.working_days = pd.concat([st.session_state.working_days, pd.DataFrame(
        {"project": name, "day": business_days, "date": project_range})])
...
def add_project(name, start, end, activities, team, sheets, planned_work, real_work, working_days):
    ...
    st.session_state.sheets = st.session_state.sheets.drop_duplicates(subset=[
                                                                      "person", "date"])
    st.session_state.real_work = pd.concat(
        [st.session_state.real_work, real_work])
    st.session_state.real_work = st.session_state.real_work.drop_duplicates(
        subset=["person", "project", "date"])
    ...
    for act in activities.itertuples(index=False):
        planned_work = planned_work.query(
            '~ (activity == @act.activity and (date < @act.real_start_date or date > @act.real_end_date))')

    st.session_state.planned_work = pd.concat(
        [st.session_state.planned_work, planned_work])

    team = team[["person", "gender"]].rename(
        {'person': 'name'}, axis='columns')
...
def read_timesheet(file):
    ...
    try:
        team = pd.read_excel(file, sheet_name="Equipa de projeto", usecols="C:G", header=7, names=[
                             "profile", "person", "gender", "start_date", "end_date"]).dropna(subset="person")
        team = pd.concat([team, pd.read_excel(file, sheet_name="Equipa de projeto", usecols=[2, 7, 8, 9, 10], header=7, names=[
                         "profile", "person", "gender", "start_date", "end_date"]).dropna(subset="person")])
    except:
        raise Exception("Erro a ler a equipa de projeto")

    try:
        timeline = pd.read_excel(file, sheet_name="Cronograma", usecols=[
                                 1, 5], header=8, names=["activity", "trl"])
    except:
    ...
    for i in range(len(wp_index)):
        ...
        wp_activities = timeline.loc[[ind for ind in timeline.index if (
            ind > wp and ind < wp_next and (ind-wp-1) % 13 == 0)], ["activity", "trl", "line"]]
        wp_activities['wp'] = wp_name

        activities = pd.concat([activities, wp_activities])

    activities = pd.merge(activities, df_colors,
                          left_on='line', right_index=True)
    try:
        activities[['start_date', 'end_date']] = activities.apply(
            lambda row: min_max_dates(row, -1, 1), axis=1, result_type='expand')
        activities[['real_start_date', 'real_end_date']] = activities.apply(
            lambda row: min_max_dates(row, 2, 1), axis=1, result_type='expand')
    except:
        raise Exception("Erro a ler as datas das atividades")

    activities.loc[pd.isna(activities['start_date']),
                   "start_date"] = start_date
    activities.loc[pd.isna(activities['end_date']), "end_date"] = end_date
    activities.loc[pd.isna(activities['real_start_date']),
                   "real_start_date"] = start_date
    activities.loc[pd.isna(activities['real_end_date']),
                   "real_end_date"] = end_date

    activities['trl'] = activities['trl'].astype(str)
    activities.loc[activities['trl'].notna(), 'trl'] = "TRL " + \
        activities['trl']
    # try:
    # except:
    #     pass

    activities = activities[['activity', 'trl', 'wp', 'start_date',
                             'end_date', 'real_start_date', 'real_end_date']]

    # Check if ther aren't any duplicated activities names
    assert not activities['activity'].duplicated().any(
    ), "Existem atividades com nomes repetidos no ficheiro"

    cols_to_convert = ['start_date', 'end_date',
                       'real_start_date', 'real_end_date']
    activities[cols_to_convert] = activities[cols_to_convert].apply(
        pd.to_datetime)
    ...
    for contract in team.itertuples():
        ...
        contract_range = pd.date_range(start=get_first_date(
            contract.start_date), end=contract.end_date, freq="MS")
        ...
        try:
            ...
            raise Exception(
                f"Folha {sheet_name}: Erro na leitura da folha de horas")
        ...
        try:
            ...
            raise Exception(
                f"Erro a ler a folha {sheet_name} - Não foi possível converter a folha com Números Inválidos")
        ...
        try:
            ...
            raise Exception(
                f"Folha {sheet_name}: Não foram encontradas as atividades")
        ...
        try:
            ...
            raise Exception(
                f"Folha {sheet_name}: Erro a converter datas da folha")
        ...
        try:
            ...
            raise Exception(
                f"Folha {sheet_name}: Não foi encontrado as Horas Reais [Horas Reais PRR]")
        ...
        try:
            ...
            raise Exception(
                f"Folha {sheet_name}: Erro a ler outras atividades")
...
def home_widget():
    ...
    project_name = st.text_input(
        "Nome do projeto", key=f"project_name_{st.session_state.key}")

    if not with_file:
        c1, c2 = st.columns(2)

        start_date = c1.date_input(
            "Data de inicio", key=f"start_date_project_{st.session_state.key}", format="DD/MM/YYYY", value=None)
        end_date = c2.date_input("Data de encerramento", key=f"end_date_project_{st.session_state.key}",
                                 format="DD/MM/YYYY", value=None, min_value=start_date + timedelta(days=1, weeks=4) if start_date else None)
    ...
    if file := st.file_uploader("Timesheet", accept_multiple_files=False, type="xlsx", key=f"file_uploader_{st.session_state.key}"):
        ...
        try:
            team, activities, sheets, planned_work, real_work, start_date, end_date, working_days = read_timesheet(
                file)
        ...
        activities_df = st.data_editor(
            activities,
            column_order=("wp", "activity", "trl", "start_date",
                          "end_date", "real_start_date", "real_end_date", "project"),
            column_config={
        ...
        print(activities_df['trl'].dtypes)

    if st.button("Adicionar Projeto", disabled=invalid(project_name, file), use_container_width=True):
        add_project(project_name, start_date, end_date, activities,
                    team, sheets, planned_work, real_work, working_days)
