...
def update_timeline(project, data, executed, to_adjust):
    ...
    st.session_state.projects.loc[st.session_state.projects["name"] ==
                                  project["name"], 'executed'] = executed.date() if executed else None

    contracts = st.session_state.contracts.query('project == @project["name"]')
    if to_adjust:
        if not executed:
            sum_act = st.session_state.planned_work.query(
                'project == @project["name"]').groupby(['person', 'activity'])['hours'].sum()
        else:
            sum_act = st.session_state.planned_work.query(
                'project == @project["name"] and date > @executed').groupby(['person', 'activity'])['hours'].sum()

        for act in data.itertuples(index=False):
            act_range = pd.date_range(start=get_first_date(
                act.real_start_date), end=get_last_date(act.real_end_date), freq="MS")

            if not executed:
                st.session_state.planned_work = st.session_state.planned_work.query(
                    '~ (project == @project["name"] and activity == @act.activity)')
            else:
                st.session_state.planned_work = st.session_state.planned_work.query(
                    '~ (project == @project["name"] and activity == @act.activity and date > @executed)')

            comb = list(product(contracts['person'], act_range))
            act_df = pd.DataFrame({"person": [item[0] for item in comb], "project": project['name'],
                                  "activity": act.activity, "date": [item[1] for item in comb], "hours": 0})
            st.session_state.planned_work = pd.concat(
                [st.session_state.planned_work, act_df])

        st.session_state.planned_work = st.session_state.planned_work.drop_duplicates(
            subset=["person", "activity", "date"])
        st.session_state.planned_work = st.session_state.planned_work.sort_values(
            by="date")
        ...
        for contract in contracts.itertuples():
            ...
            st.session_state.planned_work = st.session_state.planned_work.query(
                '(person != @contract.person) or (project != @project["name"]) or (date >= @contract_start_date and date <= @contract_end_date)')
            ...
            for act in data['activity']:
                ...
                if executed:
                    ...
                    filter = (df['person'] == contract.person) & (
                        df['date'] > pd.to_datetime(executed)) & (df['activity'] == act)

                    st.session_state.planned_work.loc[filter, 'hours'] = sum
                    st.session_state.planned_work.loc[filter, 'hours'] = st.session_state.planned_work.loc[filter, 'hours'].div(
                        len(st.session_state.planned_work.loc[filter, 'hours']))
                else:
                    df = st.session_state.planned_work
                    filter = (df['person'] == contract.person) & (
                        df['activity'] == act)
        ...
        for act in data.itertuples(index=False):
            ...
            act_range = pd.date_range(
                start=real_start_date, end=real_end_date, freq="MS")

            comb = list(product(contracts['person'], act_range))
            act_df = pd.DataFrame({"person": [item[0] for item in comb], "project": project['name'],
                                  "activity": act.activity, "date": [item[1] for item in comb], "hours": 0})
            st.session_state.planned_work = st.session_state.planned_work.query(
                '~ (activity == @act.activity and project == @project["name"] and (date < @real_start_date or date > @real_end_date))')
            st.session_state.planned_work = pd.concat(
                [st.session_state.planned_work, act_df])

        st.session_state.planned_work = st.session_state.planned_work.dropna(subset=[
                                                                             'hours'])
        st.session_state.planned_work = st.session_state.planned_work.drop_duplicates(
            subset=["person", "activity", "date"])
        st.session_state.planned_work = st.session_state.planned_work.sort_values(
        ...
        for contract in contracts.itertuples():
            ...
            st.session_state.planned_work = st.session_state.planned_work.query(
                '~ (person == @contract.person and (date < @contract_start_date or date > @contract_end_date))')

    st.session_state.planned_work = st.session_state.planned_work.query(
        '~ (project == @project["name"] and activity not in @data["activity"].unique())')
    ...
    set_notification(
        "success", "Cronograma atualizado com sucesso!", force_reset=True)


@st.cache_data
def get_gantt(project, timeline):
    result = timeline.groupby('wp').agg(
        {'start_date': 'min', 'end_date': 'max', 'real_start_date': 'min', 'real_end_date': 'max'})
...
def timeline_widget(project):
    ...
    timeline['real_start_date'] = pd.to_datetime(
        timeline['real_start_date']).dt.date
    timeline['real_end_date'] = pd.to_datetime(
        timeline['real_end_date']).dt.date
    ...
    if len(gantt_data) > 0:
        ...
        fig = px.timeline(gantt_df, x_start="Start", x_end="Finish", y="Task", color="Color", color_discrete_map={
                          'Planeado': "#0AA3EB", "Real": "#DAF1FC", "Executado": "#878787"}, category_orders={'Color': ["Planeado", "Real", "Executado"]})
        ...
        st.write('<p style="text-align: center;">Sem Dados Disponíveis</p>',
...
""", unsafe_allow_html=True)
    ...
    if col2.button(':x:'):
        st.session_state[f"date_{st.session_state.key}"] = None

    executed_date = pd.to_datetime(col1.date_input(
        "Data de execução do projeto [bloqueada]", key=f"date_{st.session_state.key}", value=project['executed'], min_value=project['start_date'], max_value=project['end_date'], format="DD/MM/YYYY"))
    ...
    if not wps:
        st.write('<p style="text-align: center;">Nenhum WP Encontrado</p>',
                 unsafe_allow_html=True)
        ...
        for wp in wps:
            ...
            with st.expander(wp, expanded=True):
                ...
                wp_acts = st.data_editor(
                    ...
                    column_order=["activity", "trl", "start_date",
    ...
    if save:
        ...
        update_timeline(project, to_update.reset_index(
            names="activity"), executed_date, to_adjust)
